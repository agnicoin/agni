...
const char * const BITCOIN_CONF_FILENAME = "agni.conf";
const char * const BITCOIN_PID_FILENAME = "agnid.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "agni" is a composite category enabling all Agni-related debug output
            if(ptrCategory->count(string("agni"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AgniCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AgniCore
    // Mac: ~/Library/Application Support/AgniCore
    // Unix: ~/.agnicore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AgniCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AgniCore";
#else
    // Unix
    return pathRet / ".agnicore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty agni.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override agni.conf
