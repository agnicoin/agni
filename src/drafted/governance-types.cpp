...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "AgniNetwork") type = AgniNetwork;
    if(strType == "AgniNetworkVariable") type = AgniNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == AgniNetwork) s = "AgniNetwork";
    if(type == AgniNetworkVariable) s = "AgniNetworkVariable";
