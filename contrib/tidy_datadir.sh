...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Agni database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Agni datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Agni datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Agni 0.7 datadir."
    ;;
  3)
    echo "Detected Agni pre-0.8 datadir."
    ;;
  4)
    echo "Detected Agni 0.8 datadir."
    ;;
esac
